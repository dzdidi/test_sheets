...
function addCalls(sheet, scheme, indent, res){
  if (scheme.length === 0) {
    return res;
  } else {
    ...
    return res.concat('  '.repeat(indent) +'nodeBase('
      ...
      , '\'' + getValue(sheet, scheme[0].objectsUnderTest) + ' ' + getValue(sheet, scheme[0].methodsUnderTest)+ '\'' + ')){\n'
      , '  '.repeat(indent + 2)
      , scheme[0].outputs
      , ' = data;\n'
      , '  '.repeat(indent + 1)
      ...
      , '  '.repeat(indent + 1)
      , '};\n'
      , addCalls(sheet, scheme.slice(1, Object.keys(scheme).length), indent + 1, res)
      , '  '.repeat(indent) + '});\n');
  };
};
