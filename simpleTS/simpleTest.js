...
nodeBase(this, 'BankAustria', C3, D3, ["getAccounts"], null, null, E3, function(err, data){
  ...
  if(makeComparisonAndWriteResult(G3, data, '||', 'BankAustria getAccounts')){
    G3 = data;
  } else {
    return;
  };
  nodeBase(this, 'BankAustria', C4, null, ["getAccounts"], null, null, null, function(err, data){
    if(err)  return(err, null);
    if(makeComparisonAndWriteResult(G4, data, '|', 'BankAustria getAccounts')){
      G4 = data;
    } else {
      return;
    };
    nodeBase(this, 'BankAustria', C5, D5, ["getAccounts"], null, null, null, function(err, data){
      if(err)  return(err, null);
      if(makeComparisonAndWriteResult(G5, data, '||', 'BankAustria getAccounts')){
        G5 = data;
      } else {
        return;
      };
